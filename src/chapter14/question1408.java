...
public class question1408 {
	...
	public static void main(String[] args) {
		...
		list.append(4);
		list.append(5);
		TreeNode<Integer> root = BSTfromLinkedList(new NodeReference<Integer>(
				list.head), 0, list.size);
		TreeNode.beautifulPrint(root);
	}

	// Necessary because node = node.next is assignment, so without the ref
	// wrapper, the change will not be reflected on node.
	...
	public static TreeNode<Integer> BSTfromLinkedList(
			NodeReference<Integer> nodeRef, int start, int end) {
		if (start >= end) {
			return null;
		}
