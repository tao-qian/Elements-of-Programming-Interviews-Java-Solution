...
public class question1527 {
	...
	public static int calculateWeightDiff(Node root, int lastWeights, int goal,
			HashMap<Node, Integer> weightDiff) {
		int diff = Integer.MAX_VALUE;
		if (root.children.size() == 0) {
			diff = goal - lastWeights - root.weight;
		} else {
			for (Node child : root.children) {
				int childWeight = calculateWeightDiff(child, lastWeights
						+ root.weight, goal, weightDiff);
