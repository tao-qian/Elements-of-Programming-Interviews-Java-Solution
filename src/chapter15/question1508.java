	...
	public static int largestRectangle(int[] h) {
		...
		for (int i = 0; i < h.length; i++) {
			while (forwardStack.size() != 0
					&& h[forwardStack.peekLast()] >= h[i]) {
				forwardStack.pollLast();
			}
			if (forwardStack.size() == 0) {
				/*
				 * When the stack is empty, there can be two cases. The first
				 * one is that we are at the beginning of the loop, in this case
				 * i = 0, and the width of i should also be 0. The second case
				 * is that we are in the middle of the loop, but h[i] is smaller
				 * than all the number in front of it, so the width should be i
				 */
				leftRecWidth[i] = i;
			} else {
				/*
				 * Otherwise the width is the difference between the current
				 * index and the last index whose height is smaller than the
				 * current height
				 */
		...
		System.out.println(maxIndex - leftRecWidth[maxIndex] + ","
				+ (maxIndex + rightRecWidth[maxIndex]));
		return max;
	}
}
