...
public class question1311 {
	...
	public static ArrayList<Interval> unionOfIntervals(ArrayList<Interval> intervals)
	{
		...
		for(int i = 0; i < intervals.size();i++)
		{
			...
			(intervals.get(i).start == current.end && !intervals.get(i).startInclusive && !current.endInclusive))
			{
				union.add(current);
				current = intervals.get(i);
				continue;
			}
			//Otherwise we update current to be an union of the old one and the new interval
